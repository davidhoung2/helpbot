...
class FreeProvider(BaseProvider):
    ...
    def __init__(self):
        ...
        # Free providers via g4f are HIGHLY UNSTABLE and may fail frequently
        # The g4f library provides free access but providers change constantly
        # For reliable service, use a premium provider (OpenAI, Claude, Gemini, Grok)
    ...
    async def chat_completion(self, messages: List[Dict[str, str]], model: str, **kwargs) -> str:
        ...
        raise Exception(
            "All free providers are currently unavailable. "
            "Free providers are unreliable and change frequently. "
            "For reliable service, please use a premium provider (OpenAI, Claude, Gemini, or Grok) "
            "by setting up an API key and switching with /provider command."
        )
    ...
    def get_available_models(self) -> List[ModelInfo]:
        """Return available free models (UNSTABLE - may fail frequently)"""
        models = [
            ModelInfo("gpt-3.5-turbo", ProviderType.FREE, "Free GPT-3.5 (unstable, use premium for reliability)"),
            ModelInfo("blackboxai", ProviderType.FREE, "Blackbox AI (free, experimental)"),
        ]
